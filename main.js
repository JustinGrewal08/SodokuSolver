...
function loadData() {
  const inputs = document.querySelectorAll(".nums");


    for(let x; x > document.querySelectorAll(".nums"); x++) {
        if (document.querySelectorAll(".nums")[x] > 9 || document.querySelectorAll(".nums")[x] < 1){
            const warningMessage = document.createAttribute("h2");
            warningMessage.setAttribute("value", "WARNING")
        }
    }
